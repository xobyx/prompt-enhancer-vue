...
import { safeEval, generateWorkflowDiagram } from '../utils/workflowUtils';
  ...
  try {
    ...
    const startingSteps = props.workflow.steps.filter(step => {
      const hasIncoming = props.workflow.steps.some(s => 
        s.conditions?.some(c => c.targetStepId === step.id)
      );
      ...
      if (readySteps.length === 0) {
        ...
        for (const step of readySteps) {
          try {
            ...
            const nextSteps = step.conditions
              ?.filter((cond: any) => {
                try {
                  return safeEval(cond.expression, { 
                    output, 
              ...
              .map((cond: any) => props.workflow.steps.find(s => s.id === cond.targetStepId))
